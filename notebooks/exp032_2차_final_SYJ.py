# ════════════════════════════════════════════════════════════
# exp032 2차 최종 — Mixed FE (팀원 파라미터 전체 적용)
#   ① LGB : FE-v1 + 팀원 EXP-019 파라미터
#   ② CAT : FE-v2 + fold-wise TE + 팀원 EXP-011 파라미터
#   ③ XGB : FE-v2 + fold-wise TE + ITE + 팀원 EXP-012 파라미터
#   ④ 앙상블: Rank Average + Optuna 가중치 비교
#
# OOF AUC: 0.74037 (최고)
# 기준선: exp024 OOF 0.74025 / 제출 0.74169
# ════════════════════════════════════════════════════════════
...
# LGB — 팀원 EXP-019 (LGBMClassifier 키 변환)
BEST_LGB_PARAMS = {
    "n_estimators": 2000,
...
# CAT/XGB용: FE-v2 + LabelEncoding (TE/ITE는 fold 안에서)
...
test_cat = np.zeros(len(test_v2_enc))
test_xgb = np.zeros(len(test_v2_enc))

print(f"\n{'='*60}")
print(f"exp032 2차 OOF 학습 (팀원 파라미터 전체, 5-Fold)")
print(f"{'='*60}")

for fold, (tr_idx, val_idx) in enumerate(skf.split(train_v2_enc, y_all), 1):
