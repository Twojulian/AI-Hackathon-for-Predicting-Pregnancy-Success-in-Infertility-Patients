...
print(f"\nOOF AUC  CAT={auc_cat:.5f}  XGB={auc_xgb:.5f}")
...
oofs  = np.stack([  oof_cat,  oof_xgb],  axis=1)
tests = np.stack([ test_cat, test_xgb], axis=1)
aucs  = np.array([ auc_cat, auc_xgb])
...
best_w = np.array([study.best_params["w_cat"],
                   study.best_params["w_xgb"]])
best_w = best_w / best_w.sum()
print(f"  최적 가중치: CAT={best_w[0]:.3f}  XGB={best_w[1]:.3f}")
...
print(f"\n{'='*65}")
print(f"  개별: CAT={auc_cat:.5f}  XGB={auc_xgb:.5f}")
...
print(f"모델명        : CAT(FE-v2+TE) + XGB(FE-v2+TE+ITE)")
...
print(f"Optuna 가중치: CAT={best_w[0]:.3f}  XGB={best_w[1]:.3f}")
...
print(f"인사이트     : 팀원 EXP-032 방식 적용, exp031 리크 문제 수정")
print("="*55)
