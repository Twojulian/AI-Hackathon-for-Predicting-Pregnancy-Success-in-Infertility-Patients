...
#   ① 피처: exp024 기준 102개
#   ② LGB/CAT/XGB: exp024 Optuna 최적 파라미터 그대로
#   ③ ExtraTrees: Optuna로 간단 튜닝 후 추가
#   ④ 앙상블: Rank Average (4모델)
#
...
# ExtraTrees Optuna 탐색 횟수 (빠르게 하려면 30, 정밀하게 하려면 80)
ET_N_TRIALS = 50
...
X_submit = test_df.drop(columns=[TARGET], errors="ignore").reindex(columns=X_all.columns, fill_value=0)
...
print(f"\n{'='*60}")
print(f"[ExtraTrees Optuna 튜닝] {ET_N_TRIALS}회 탐색")
print(f"{'='*60}")
...
def et_objective(trial):
    ...
    for tr_idx, val_idx in skf.split(X_all, y_all):
        ...
        et.fit(X_all.iloc[tr_idx], y_all.iloc[tr_idx])
        oof[val_idx] = et.predict_proba(X_all.iloc[val_idx])[:, 1]
    return roc_auc_score(y_all, oof)

study_et = optuna.create_study(direction="maximize",
                               sampler=optuna.samplers.TPESampler(seed=SEED))
study_et.optimize(et_objective, n_trials=ET_N_TRIALS, show_progress_bar=True)

BEST_ET_PARAMS = study_et.best_params
BEST_ET_PARAMS.update({"class_weight": "balanced", "random_state": SEED, "n_jobs": -1})
print(f"\nExtraTrees 최적 파라미터: {BEST_ET_PARAMS}")
print(f"ExtraTrees 단독 OOF AUC: {study_et.best_value:.5f}")
...
test_et  = np.zeros(len(X_submit))

print(f"\n{'='*60}")
print(f"exp030 OOF 학습 (LGB + CAT + XGB + ET, 5-Fold)")
print(f"{'='*60}")
...
for fold, (tr_idx, val_idx) in enumerate(skf.split(X_all, y_all), 1):
    ...
    test_et         += et_model.predict_proba(X_submit)[:, 1] / N_FOLDS
    ...
    print(f"  Fold {fold}  LGB={roc_auc_score(y_val, oof_lgb[val_idx]):.4f}"
          ...
          f"  ET={roc_auc_score(y_val, oof_et[val_idx]):.4f}")

auc_lgb = roc_auc_score(y_all, oof_lgb)
auc_cat = roc_auc_score(y_all, oof_cat)
auc_xgb = roc_auc_score(y_all, oof_xgb)
auc_et  = roc_auc_score(y_all, oof_et)
print(f"\nOOF AUC  LGB={auc_lgb:.5f}  CAT={auc_cat:.5f}  XGB={auc_xgb:.5f}  ET={auc_et:.5f}")
...
oofs_4  = np.stack([oof_lgb,  oof_cat,  oof_xgb,  oof_et],  axis=1)
tests_4 = np.stack([test_lgb, test_cat, test_xgb, test_et], axis=1)
oofs_3  = np.stack([oof_lgb,  oof_cat,  oof_xgb],           axis=1)
tests_3 = np.stack([test_lgb, test_cat, test_xgb],          axis=1)
...
results = {
    "Rank Average 3모델 (비교용)": (
        roc_auc_score(y_all, rank_avg(oofs_3)),
        rank_normalize(rank_avg(tests_3)),
    ),
    "Rank Average 4모델": (
        roc_auc_score(y_all, rank_avg(oofs_4)),
        rank_normalize(rank_avg(tests_4)),
    ),
    "Simple Average 4모델": (
        roc_auc_score(y_all, oofs_4.mean(axis=1)),
        oofs_4.mean(axis=1),
    ),
}

print(f"\n{'='*65}")
print(f"  개별: LGB={auc_lgb:.5f}  CAT={auc_cat:.5f}  XGB={auc_xgb:.5f}  ET={auc_et:.5f}")
...
for method, (auc, test_pred) in results.items():
    ...
    print(f"  {method:30s}: {auc:.5f}  ({diff:+.5f} vs exp024){flag}")
    if auc > best_auc:
...
oof_best   = rank_avg(oofs_4) if "4모델" in best_method else rank_avg(oofs_3)
oof_binary = (oof_best >= np.percentile(oof_best, 70)).astype(int)

print("\n" + "="*55)
print("📋 실험 기록장 정보")
print("="*55)
print(f"실험 번호     : exp{EXP_NO:03d}")
print(f"모델명        : LGB + CAT + XGB + ExtraTrees 앙상블")
print(f"앙상블 방법   : {best_method}")
print(f"전체 피처 수  : {X_all.shape[1]}  /  LGB 피처 수: {X_all_lgb.shape[1]}")
print(f"ET Optuna 횟수: {ET_N_TRIALS}회")
print(f"LGB OOF AUC  : {auc_lgb:.5f}")
print(f"CAT OOF AUC  : {auc_cat:.5f}")
print(f"XGB OOF AUC  : {auc_xgb:.5f}")
print(f"ET  OOF AUC  : {auc_et:.5f}")
print(f"최적 ET 파라미터: {BEST_ET_PARAMS}")
...
print(f"클래스 불균형 : class_weight=balanced (ET) / scale_pos_weight={neg_pos_ratio:.4f} (LGB/CAT/XGB)")
print(f"파일명        : {out_fname}")
print(f"특이사항      :")
print(f"  - 피처 구성: exp024 기준 102개")
print(f"  - ET 추가로 앙상블 다양성 확보 시도")
print(f"  - 3모델 vs 4모델 Rank Average 비교 포함")
print(f"인사이트      : ExtraTrees 추가가 앙상블 다양성에 기여하는지 확인 목적")
print("="*55)
