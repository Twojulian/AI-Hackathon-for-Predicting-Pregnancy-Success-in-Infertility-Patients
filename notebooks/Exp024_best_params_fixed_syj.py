# ════════════════════════════════════════════════════════════
# exp024 — 베스트 파람 고정 버전 (Optuna 없음)
#          Optuna 탐색 결과를 하드코딩 → 바로 OOF 학습 + 앙상블
#          전처리: exp023/024와 완전 동일 (DI 구조적 결측 처리 + 102 피처)
#
# ★ 베스트 파람 출처
#   LGB: n_trials=100, Best OOF AUC=0.73993
#   CAT: n_trials=30,  Best OOF AUC=0.73994
#   XGB: n_trials=50,  Best OOF AUC=0.73997
#   앙상블 OOF AUC=0.74025 (Rank Average)
#
# 기준선: exp023 (OOF 0.73997)
# ════════════════════════════════════════════════════════════
...
BEST_LGB_PARAMS = {
    ...
    # scale_pos_weight는 데이터 로드 후 자동 계산되어 추가됩니다
    "random_state":      SEED,
    "n_jobs":            -1,
    "verbose":           -1,
}
...
# scale_pos_weight 파람에 자동 반영
BEST_LGB_PARAMS["scale_pos_weight"] = neg_pos_ratio
BEST_CAT_PARAMS["scale_pos_weight"] = neg_pos_ratio
BEST_XGB_PARAMS["scale_pos_weight"] = neg_pos_ratio


# ════════════════════════════════════════════════════════════
# OOF 학습 + test 예측 (Optuna 없이 바로 돌림)
# ════════════════════════════════════════════════════════════
...
print(f"\n{'='*60}")
print(f"exp024 베스트 파람으로 OOF 학습 (Optuna 없음)")
print(f"{'='*60}")
...
test_rank_mean = test_ranks.mean(axis=1)
test_rank_norm = (test_rank_mean - test_rank_mean.min()) / \
                 (test_rank_mean.max() - test_rank_mean.min())

results["Rank Average"] = (
    roc_auc_score(y_all, oof_ranks.mean(axis=1)),
    oof_ranks.mean(axis=1),
    test_rank_norm,
)
...
for method, (auc, oof_pred, test_pred) in results.items():
    flag = " ← best" if auc == max(v[0] for v in results.values()) else ""
    diff = auc - BASELINE
...
print(f"특이사항      : Optuna 탐색 없이 exp024 베스트 파람 하드코딩으로 재현")
print(f"  LGB: n_trials=100 best / CAT: n_trials=30 best / XGB: n_trials=50 best")
print("="*55)
