...
# RF + LGB 앙상블 Val AUC 확인
best_lgb_params = {'n_estimators': 400, 'learning_rate': 0.04956090868322492, 
                    'num_leaves': 112, 'max_depth': 4, 'min_child_samples': 26, 
                    'subsample': 0.7523886151586271, 'colsample_bytree': 0.6353997510251372, 
                    'reg_alpha': 5.682819774457299, 'reg_lambda': 8.955144228161172,
...
rf_auc = roc_auc_score(y_val, final_rf.predict_proba(X_val)[:, 1])

# 앙상블
ensemble_v1 = (final_rf.predict_proba(X_val)[:, 1] * 0.6 +
               final_lgb.predict_proba(X_val)[:, 1] * 0.4)

ensemble_v2 = (final_rf.predict_proba(X_val)[:, 1] * 0.4 +
               final_lgb.predict_proba(X_val)[:, 1] * 0.6)

# 균등 비교도 같이
ensemble_v3 = (final_rf.predict_proba(X_val)[:, 1] * 0.5 +
               final_lgb.predict_proba(X_val)[:, 1] * 0.5)

print(f"RF Val AUC:                {rf_auc:.4f}")
print(f"LGB Val AUC:               {lgb_auc:.4f}")

print(f"앙상블 v1 (RF0.6+LGB0.4):  {roc_auc_score(y_val, ensemble_v1):.4f}")
print(f"앙상블 v2 (RF0.4+LGB0.6):  {roc_auc_score(y_val, ensemble_v2):.4f}")
print(f"앙상블 v3 (RF0.5+LGB0.5):  {roc_auc_score(y_val, ensemble_v3):.4f}")
...
final_rf_full = RandomForestClassifier(
    n_estimators=300, max_depth=10, min_samples_split=2,
    min_samples_leaf=9, max_features=0.7,
    class_weight="balanced", random_state=42, n_jobs=-1
)
final_rf_full.fit(X, y)

# v1 제출 파일
ensemble_submit_v1 = (final_rf_full.predict_proba(X_submit)[:, 1] * 0.6 +
                      final_lgb_full.predict_proba(X_submit)[:, 1] * 0.4)
pd.DataFrame({"ID": test_ids, "probability": ensemble_submit_v1})\
    .to_csv("submission_exp018_v1_SYJ.csv", index=False)

# v2 제출 파일
ensemble_submit_v2 = (final_rf_full.predict_proba(X_submit)[:, 1] * 0.4 +
                      final_lgb_full.predict_proba(X_submit)[:, 1] * 0.6)
pd.DataFrame({"ID": test_ids, "probability": ensemble_submit_v2})\
    .to_csv("submission_exp018_v2_SYJ.csv", index=False)

# v3 제출 파일
ensemble_submit_v3 = (final_rf_full.predict_proba(X_submit)[:, 1] * 0.5 +
                      final_lgb_full.predict_proba(X_submit)[:, 1] * 0.5)
pd.DataFrame({"ID": test_ids, "probability": ensemble_submit_v3})\
    .to_csv("submission_exp018_v3_SYJ.csv", index=False)
